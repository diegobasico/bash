...
# Function to run tmux command as rtorrent user with proper socket and terminal
run_tmux() {
  sudo -u "$RTORRENT_USER" env HOME="$RTORRENT_HOME" SHELL=/bin/bash TERM=xterm-256color tmux -S "$TMUX_SOCKET" "$@"
}
...
debug_tmux() {
  ...
  echo "Script will use TERM: xterm-256color"
  ...
  echo -e "\nTesting basic tmux functionality with proper environment:"
  sudo -u "$RTORRENT_USER" env HOME="$RTORRENT_HOME" SHELL=/bin/bash TERM=xterm-256color tmux -S "$TMUX_SOCKET" new-session -d -s "test-session" /bin/bash 2>&1
  if sudo -u "$RTORRENT_USER" env HOME="$RTORRENT_HOME" SHELL=/bin/bash TERM=xterm-256color tmux -S "$TMUX_SOCKET" has-session -t "test-session" 2>/dev/null; then
    echo "Test session created successfully"
    sudo -u "$RTORRENT_USER" env HOME="$RTORRENT_HOME" SHELL=/bin/bash TERM=xterm-256color tmux -S "$TMUX_SOCKET" kill-session -t "test-session" 2>/dev/null
...
# Function to attach to session remotely (for systems without proper terminal)
attach_remote() {
  if ! session_exists; then
    print_error "rtorrent tmux session does not exist. Start it first with: $0 start"
    return 1
  ...
  print_info "Attaching to rtorrent tmux session (remote-friendly)..."
  print_info "To detach: Ctrl+b then d"
  print_info "To quit rtorrent: Ctrl+q"
  echo

  # Use a more compatible terminal setting for remote access
  sudo -u "$RTORRENT_USER" env HOME="$RTORRENT_HOME" SHELL=/bin/bash TERM=xterm-256color tmux -S "$TMUX_SOCKET" attach-session -t "$TMUX_SESSION"
}
...
show_help() {
  ...
  echo
  echo "Usage: $0 {start|stop|restart|status|attach|attach-remote|logs|send|setup|debug|help}"
  echo
  echo "Commands:"
  ...
  echo "  attach-remote- Attach with explicit terminal settings"
  ...
  echo "  $0 attach-remote            # Attach with explicit terminal"
